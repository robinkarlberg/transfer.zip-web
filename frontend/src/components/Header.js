...
export default function Header() {
  ...
  const ctaText = useMemo(() => isGuestUser ? "Sign Up!" : "Dashboard", [isGuestUser])
  const ctaLink = useMemo(() => isGuestUser ? "/signup" : "/app", [isGuestUser])
    ...
    <header className="backdrop-blur bg-gray-50 bg-opacity-70 fixed top-0 left-0 w-full z-10 border-b">
      <nav aria-label="Global" className="mx-auto flex max-w-7xl items-center justify-between p-4 lg:px-8">
        ...
        <div className="hidden lg:flex lg:flex-1 lg:justify-end">
          {
            isGuestUser ?
              <Link onClick={handleCtaLinkClicked} to={ctaLink} className="text-sm/6 font-semibold text-white rounded-full bg-primary px-3 py-0.5 hover:bg-primary-light">
                {ctaText} <span aria-hidden="true">&rarr;</span>
              </Link>
              :
              <Link onClick={handleCtaLinkClicked} to={ctaLink} className="text-sm/6 font-semibold text-gray-900">
                {ctaText} <span aria-hidden="true">&rarr;</span>
              </Link>
          }
