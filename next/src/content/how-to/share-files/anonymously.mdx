...
- **Files can be streamed in real time (Quick Transfer)**, meaning nothing is saved on servers if you don't want.


![Transfer.zip Screenshot](/img/content/services/transfer-zip.png)
...
<PrimaryLink href="/">Send files anonymously with Transfer.zip</PrimaryLink>
...
## Tips for Anonymous Sharing

1. **End-to-end encryption** - pick a service that encrypts in the browser before upload, so the host can't read your files.
2. **Temporary links** - set short expirations or use a service that auto-deletes after download.
3. **No account, no email** - if a service asks for a "real" email, it's not anonymous.
4. **Double-check the URL** - phishing copies of file transfer sites are common.
