...
export default function ({ user, storage, variant }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className={`mx-auto w-full sm:shadow-sm bg-white ${compact ? "max-w-80 rounded-2xl border" : "max-w-96 sm:rounded-2xl sm:border"}`}>
        {!compact && (
          <div className="flex justify-center mt-2 sm:mt-4">
            <fieldset aria-label="Transfer direction">
              <RadioGroup
                value={direction}
                onChange={value => {
                  router.push(`?dir=${value}`, { scroll: false })
                }}
        ...
        <form onSubmit={e => e.preventDefault()} ref={formRef} className="grid grid-cols-3 gap-y-6 gap-x-2 px-6 mt-4">
            ...
            <div className="relative mt-2 flex items-center">
              ...
              <div className="absolute inset-y-0 right-0 flex py-1.5 pr-1.5">
                <button type="button" onClick={handleEmailAdd} className="inline-flex items-center rounded border border-gray-200 px-1 pe-1.5 font-sans text-xs text-primary font-medium bg-white hover:bg-gray-50">
                  <BIcon name={"plus-lg"} className={"mr-1 ms-1"} />Add{!compact && " Email"}
                </button>
              </div>
            </div>
