...
export class FileTransfer {
    ...
    sendFile(file, cbProgress, cbFinished) {
        return new Promise((resolve, reject) => {
            ...
            this.channel.addEventListener("message", async e => {
                ...
                if (data.type == "progress") {
                    ...
                    if (data.message == "cancel") {
                        reject("User canceled the download")
                    }
                    else {
            ...
            const readSlice = o => {
                // console.log("readSlice", o, this.channel.bufferedAmount)

                if (this.channel.bufferedAmount > 5000000) {
                    // console.log("WAIT", this.channel.bufferedAmount)
            ...
            this.sendAndEncryptPacket(packet).then(() => {
                readSlice(0)
            })
        })
    }

    recvFile(cbFileInfo, cbProgress, cbFinished) {
        return new Promise((resolve, reject) => {
            ...
            const handleChunkMap = () => {
                ...
                if (writer.desiredSize == null) {
                    ...
                    this.channel.send(JSON.stringify({ type: "error", message: "cancel" }))
                    this.channel.close()
                    reject("user cancelled the download")
                    return
                }
                if (!fileInfo) {
